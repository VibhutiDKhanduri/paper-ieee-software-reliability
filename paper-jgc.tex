...
%\title{Evolution of Processes in Delivering Reliable Software within the Distributed European Research Infrastructures%\thanks{Grants or other notes
...
\title{INDIGO-DataCloud: a converging evolution of software engineering practices to support European Research e-Infrastructures}
%\subtitle{}
...
addressed in Europe through dedicated software development projects for
e--Infrastructure creation, operation and management.
This work presents the most significant software quality breakthroughs obtained in
one of such projects, {\sl INDIGO--DataCloud}, the main challenges and barriers 
confronted throughout the lifespan of the project, and how they were partially or 
totally overcome.
The knowledge base established throughout the last 15 years 
of diverse software development initiatives in Europe for sustaining distributed research 
e-Infrastructures, supported by the advances in the area of software engineering, definitely
%constituted a significant breakthrough in the reliability of the software delivered in the 
contributed to improve the reliability of the software delivered, and consequently, the 
operational stability of the European e--Infrastructures. {\sl INDIGO--DataCloud} project is 
a good evidence of such insights, where, unlike the preceding trend found in past projects,
the enforcement of Software Quality Assurance practices has been present since the very early 
...
Throughout the last 15 years, Europe has have continuously invested on software development initiatives to provide
research e--Infrastructures with the capability of supporting unified access to large--scale computing and intense 
data analysis facilities. The developed tools have enabled the federation of geographically distributed resource 
providers, with the ultimate aim of supporting the growing needs of the scientific comumunities. According to their
requirements, those research e--Infrastructures have been evolving, exploiting diverse distributed computing 
technologies namely Grid, in the first place, and, eventually, Cloud.

The urging needs of providing stable e--Infrastructures, which were based on novel distributed computing technologies, 
...
The fundamental shift brought by the {\sl INDIGO--DataCloud} SQA procedures was 
...
%In this paper, we outline the set of EC--funded projects, ranging from 2001 to nowadays,
%that have raised the awareness and, at the same time, laid the ground work for the establishment of 
%thorough SQA practices to guide the software development lifecycle within the {\sl INDIGO--DataCloud}
%project. 
%%
%%For each project, the challenges, the methodologies and the achievements towards software 
%%reliability are described.
%%During the last 15 years the provision of European e--Infrastructures,
%%supporting unified access to large--scale computing and intense data analysis
%%applications have been driven by the needs of scientific communities as part of their
%%pan--European research collaborations. 
...
%
%A significant number of the innovation projects and technology transfer
%activities presented in Section~\ref{sec:ev}
...
%
%As a result of the analysis the aforementioned projects, 
...
%
%The former know-how combined with the continuous advancements in sofware engineering 
%have been the catalysts for drawing attention to the relevance of SQA ..
...
\subsection{Software quality validation and delivery automation}\label{software-automation}
\label{sec:devops}
...
reported by end users within the EGI e--Infrastructure, where core products were 
deployed. The number of bugs detected in the pre--release phase, filed by the SQA
responsibles of the project, surpassed by a factor of 30 the ones reported by those end 
users, which in absolute terms, this value reached no more than 200 bugs over the
observed timeframe. Considering that these low rate of bugs were uncovered throughout 
the testing phase in the project's preview testbed -- where the new releases were 
deployed in order to validate their integration with other component's production 
versions --, one can infer that the previous phase of code analysis -- running unit and
functional test cases for each minor change -- considerably helped in preventing the 
emergence of a greater quantity of software defects. As it was described, this latter
phase of code analysis included as well the compliance with de--facto style standards, 
improving the readability of the code and, consequently, building on the sustainability
...
% ?? (related defect increase?, resulting from the INDIGO-2 release?)

All along the project we have observed that source code review is the cornerstone to
improve the completeness of the unit test cases, as a commitment to strenghten the 
structural quality of the software. Therefore, we move forward in avoiding the 
aforementioned risks of having high coverage values obtained from meaningless test 
cases, which execute a great amount of statements and lines of the code, but using
incomplete test cases. Code reviewers analyse the content of each change in the 
source code and the suitability -- in the above terms -- of the associated tests, 
being the last checkpoint before the change is added in the production branch. Hence 
the vital importance, as mentioned earlier, of providing the tests alongside the code
for every given change. As we have seen, the production branches of 
{\sl INDIGO--DataCloud} source code repositories were protected against direct pushes,
requiring a manual assessment step before approval through peer code review.
% ?? figure with code reviews or sth about PRs, external participation

Functional quality, on the other hand, was assessed within the {\sl INDIGO--DataCloud} 
software lifecycle through the per--change execution of the functional tests provided 
by the developers and the ultimate stage of integration testing in the aforementioned 
preview testbed. As we have seen, the latter testing strategy was entirely performed 
in a manual way, and appeared as a helpful tool to spot defects after exposing the 
deployed software to interactions with external components. Additionally, the functional 
tests, were primarily useful to check that the software met the design specifications, 
and most importantly, that previously working functionalities are preserved when new 
features are added. As mentioned, these type of testing is meant to be continuously 
executed in an automated manner using the CI infrastructure. However, by the time of
{\sl INDIGO--1} only 30\% of the total of products provided automated functional tests,
reaching almost 60\% in {\sl INDIGO--2}. Although test reports were asked for those 
products that did not provide automated functional tests, tracking the fulfillment of 
their fundamental functional requirements was tougher a change--based approach, 
especially when assessing the regressions. The major issues observed that hindered the
implementation of automated functional tests were releated to the unavailability, at
the time, of libraries and frameworks for performing specific actions -- such as 
testing graphical interfaces -- or due to the code owners' neglect -- insufficient
knowledge of the tools -- or rejection -- time--consuming task --. As a result of this,
those products eventually -- even before {\sl INDIGO--1} -- provided functional test
reports prior to the release, thus being moved away from the CI scenario. We infer
from this experience that automated functional tests should be a goal for modern
software since they complement the structural quality checks by adding the validation
of the software's identified functionalities to improve the effectiveness of the 
DevOps approach.
% figure with % of products with functional tests

As a final remark, we want to highlight the practical experience resulted from the
application of the formerly described software engineering practices to improve the 
validation of a specific application, selected from the existing use cases in the 
{\sl INDIGO--DataCloud} project, thus expanding the scope beyond the core products. 
The outcome was a prototype CI/CD pipeline that analysed the code, packaged and 
tested the application based on a pre--defined set of inputs and reference outputs. 
The pipeline delivered then the new version only if all the stages were successful
completed. Research communities are not usually experts in programming or in related 
software engineering practices, so this fact was a real accomplishment in terms of 
the quality and sustainability of their software.

% ?? products not developed from scratch challenged the application of SQA policies??
...
%
%
%In this regard, a pre--production validation,
...
The software lifecycle demands from a continuous measurement and analysis process, 
which allows timely reactions in order to discover and fix defects throughout the
development, release and maintenance stages. Conscientious metric tracking and 
response sits at the core of an appropriate software maturity, as ultimate reliability
can only be achieved by enforcing and empowering the SQA criteria through each stage
of the software lifecycle. Lessons learned show that sustainability is a key pursuit.
The achievements, in terms of processes, knowledge, software and infrastructures, shall
be profited from former successful completed projects. In this regard, 
{\sl INDIGO-DataCloud} may have set a path for future reference in software development
practices for the European research e--Infrastructures. The SQA criteria, describing 
the quality requirements that might be considered during the entire software lifecycle,
have been published as open access and periodically reviewed to incorporate new 
recommendations and best practices. Successful software products created during the
lifetime of the project are maintained leveraging the same source code repositories and
CI infrastructure. A seamless knowledge transfer among subsequent software development
initiatives is key for the good progress of delivering reliable software.
