...
\item \textbf{Unit and Functional testing}.
...
\item \textbf{Documentation and Automated Deployment}.
...
\textit{documentation is treated as code} so that it is portable and easily reviewed, using the same workflow
as the code does, validating the changes before being updated into the production repository. On the
other hand, the software component deployment is requested to be automated using a configuration 
management tool. These tools allow to have an \textit{infrastructure as code}, managing the component's
deployment through declarative definitions. The definition files appear as self-documenting 
code, as they sequentially guide the component's deployment process in multiple platforms.
\end{itemize}
...
approved if they are not successfully executed.

Figure \ref{fig:fig_jenkins_CI_builds}


The described CI approach is guiding the project’s software development phase throughout
the first and second major releases. Throughout its lifetime, 
% FIXME complement with the paragraph above and the references to the figures for code style and unit testing!!!!
%The implementation of a CI scenario is accomplished by automating the execution of
%the test types defined in the SQA criteria described in Section \ref{subsec:sqa}. 
%Each change in the source code will trigger the associated with the software component.
%As each change is validated, the chances of early detection of defects increases. Within
%this scenario, the cost of defect solving is dramatically reduced and the reliability of the 
%software solutions improved, as any bug or design issue is likely to be detected and subsequently
%corrected at this phase.
%
