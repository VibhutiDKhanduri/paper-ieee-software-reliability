...
On behalf of the quality assurance group, specific measures for the software development process and quality assurance were defined and introduced in the project. Through project monitoring and reporting mechanisms there was a successful control of the risks, efforts and document produced. The definition of a project developer's guide \cite{datagridguide} was able to provide developers with guidelines for the various software development phases (such as packaging, test and validation process, and style and naming convention) and to allow quality group to control software release via a procedure checklist.
The specification of quality indicators \cite{datagridindicators} allowed project managers to have an update view of the quality of the services over months. Figure \ref{fig:efficiency} shows the efficiency performance indicator measured as the number of jobs successfully completed divided the total number of jobs submitted: generally the project managers observed a positive trend of efficiency over months with only one exception for the LCG1 experiment due to some errors on the dedicated resources. Figure \ref{fig:bugzilla} shows the Bugzilla anomalies follow up in the second release.
\begin{figure}[ht]
\centering
\includegraphics[width=0.5\textwidth, height=50mm]{images/dgeff.png}
\caption{Efficiency \% = Number of jobs successfully completed / Total Number of jobs submitted: results are provided for five experiments involved in the projects.}
\label{fig:efficiency}
\end{figure}

\begin{figure}[ht]
\centering
\includegraphics[width=0.5\textwidth, height=50mm]{images/dgbug.png}
\caption{Bugzilla anomalies follow up: new and pending anomalies are reported.}
\label{fig:bugzilla}
\end{figure}
...
with the goal of maintaining and evolving the different Grid middleware solutions, 
focusing on extending their interoperability and improving the reliability of the services. The
...
as well as for metric tracking, making queries on the collected data in order to display
...
The impact of the quality assurance activities was observed in the software provided by EMI to the EGI--InSPIRE project. Data reported in Table \ref{tab:emi} (see page 13 in \cite{emi-quality-model}) summarizes the EMI software quality evolution per project quarters (PQ) as evaluated by the EGI project with the help of the UMD QC. It is possible to observe the improvement of EMI releases over time that met the required quality criteria by considering the products that have passed the UMD QC, the products that have passed the EGI Stage Rollout (SR) phase (i.e. it consisted of testing the certified updates of the middleware by a subset of sites before made them available to all sites), and the products rejected because they did not meet the UMD QC.
\begin{table}[!h]
\renewcommand{\arraystretch}{1.3}
...
SR process, before its inclusion in the EGI's Cloud Middleware Distribution (CMD).
